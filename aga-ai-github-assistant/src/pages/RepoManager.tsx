...
import { listRepos, getContents, pushFile, deleteFile } from '@/lib/api'
...
export default function RepoManager() {
  ...
  useEffect(() => {
    listRepos(auth.github_token, auth.groq_key).then((r) => setRepos(r.data as Repo[]))
  }, [auth.github_token, auth.groq_key])

  const loadContents = (repo: string, p = '') => {
    getContents(auth.github_token, repo, p)
      .then((r) => setContents(r.data as RepoItem[]))
  ...
  const handlePush = async () => {
    if (!selectedRepo || !targetPath || !content)
      return toast.error('Missing repo, path, or content')
    setPushing(true)
    try {
